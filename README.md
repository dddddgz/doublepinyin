...
<table>
  ...
  <tr><td>修复重复使用同一个变量名引起的 bug</td></tr>
  ...
  <tr><td>为了方便编辑器解释，在函数的注释里添加了空行</td></tr>
  <tr>
    <td rowspan=5>2025</td>
    <td rowspan=5>5</td>
    <td rowspan=5>7</td>
    <td rowspan=5>1.5.0</td>
    <td>添加了日志功能</td>
  </tr>
  <tr><td>将记录可用汉字和它们对应拼音的 chars2.txt 重命名为 chars.txt</td></tr>
  <tr><td>加入了正确率统计功能</td></tr>
  <tr><td>对找不到程序所需文件的情况加入了容错机制</td></tr>
  <tr><td>对“新方案”页面进行了重新设计，调整了窗口布局，错误信息通过一个 Label 而不是提示框反馈给用户。</td></tr>
  <tr>
    <td>2025</td>
    <td>5</td>
    <td>11</td>
    <td>1.5.1</td>
    <td>修复键位图上 k 键没有对应声母 k 的 bug</td>
  </tr>
</table>
...
- *已解决*：bug 初次解决后被“鞭尸”
- ~~已解决~~：通过删除出现 bug 的功能解决
- **未解决**：bug 尚未解决
- 无需解决：版本号等随着更新自然消失的 bug
...
|  5  | 键道双拼下 `j` `q` `x` `y` 后的 `ü` 没有使用 `u` 对应按键 |  1.2.0   |  1.2.1   |   已解决   |
...
| 10  | 键位图上 `k` 键没有显示声母 `k`                           |  1.3.1   |  1.5.1   |   已解决   |
| 11  | 显示双拼按键的功能无法立刻取消（要等到下一个字才取消）    |  1.3.3   |  1.4.0   |   已解决   |
| 12  | 显示双拼按键的功能无法根据双拼方案的改变自动改变          |  1.3.3   |  1.4.0   |   已解决   |
| 13  | 根据音节寻找按键的功能无法在找不到时显示 `-`              |  1.3.?   |  1.5.0   |   已解决   |
| 14  | 1.4.0 的版本号仍然显示 1.3.3                              |  1.4.0   |    -     |  无需解决  |
